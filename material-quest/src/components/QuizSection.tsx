...
export default function QuizSection() {
  ...
  function handleSelect(idx: number) {
    ...
    setAnswers((prev) => ({
      ...prev,
      [current]: { selected: idx, correct: idx === q.correctIndex },
    }));
  }
        ...
        <AnimatePresence mode="wait">
          {isComplete ? (
            ...
            >
              {/* Score hero */}
              <div
                className={`rounded-2xl border-2 p-8 text-center mb-8 ${level.bg} ${level.border}`}
              >
                ...
                <div className={`text-6xl font-bold mb-2 ${level.color}`}>
                  {pct}%
                </div>
              ...
              <div className="flex justify-center gap-2 mb-8">
                {[1, 2, 3].map((s) => (
                  <Star
                    key={s}
                    className={`w-8 h-8 ${pct >= s * 33 ? "text-amber-400 fill-amber-400" : "text-slate-200 fill-slate-200"}`}
                  />
                ))}
              </div>
              ...
              <div className="space-y-2 mb-8">
                {quizQuestions.map((q, idx) => {
                  ...
                  return (
                    <div
                      key={q.id}
                      className={`flex items-start gap-3 px-4 py-3 rounded-xl ${ans?.correct ? "bg-emerald-50" : "bg-rose-50"}`}
                    >
                      {ans?.correct ? (
            ...
            >
              ...
              <div className="space-y-3 mb-5">
                {q.options.map((opt, idx) => {
                  let cls =
                    "w-full text-left px-5 py-4 rounded-2xl border-2 text-sm font-medium transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ";

                  if (!currentAnswer) {
                    cls +=
                      selected === idx
                        ? "border-blue-900 bg-blue-50 dark:bg-blue-900/30 text-blue-900 dark:text-blue-300"
                        : "border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:border-blue-300 hover:bg-blue-50 dark:hover:bg-slate-700";
                  } else if (idx === q.correctIndex) {
                    cls += "border-emerald-400 bg-emerald-50 text-emerald-800";
                  } else if (
                    idx === currentAnswer.selected &&
                ...
                {currentAnswer && (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span
                        className={`font-bold ${currentAnswer.correct ? "text-emerald-800" : "text-rose-800"}`}
                      >
