...
const elementStyles: Record<string, { bg: string; border: string; glow: string; charColor: string }> = {
  kim:  { bg: 'bg-yellow-50',  border: 'border-yellow-300', glow: 'shadow-yellow-200', charColor: 'text-yellow-600' },
  moc:  { bg: 'bg-green-50',   border: 'border-green-300',  glow: 'shadow-green-200',  charColor: 'text-green-600' },
  thuy: { bg: 'bg-blue-50',    border: 'border-blue-300',   glow: 'shadow-blue-200',   charColor: 'text-blue-600' },
  hoa:  { bg: 'bg-red-50',     border: 'border-red-300',    glow: 'shadow-red-200',    charColor: 'text-red-600' },
  tho:  { bg: 'bg-amber-50',   border: 'border-amber-300',  glow: 'shadow-amber-200',  charColor: 'text-amber-600' },
};
        ...
        {selected && (
          <motion.div key={selected.id}
            ...
            <div className="flex items-center gap-4 mb-4">
              <span className={`text-5xl font-bold ${elementStyles[selected.id].charColor}`}>{selected.character}</span>
              <div>
                <h4 className="text-xl font-bold text-slate-800">{selected.name}</h4>
                <p className="text-sm text-slate-500">Ngũ hành · Triết học Trung Hoa</p>
              </div>
            </div>
            <p className="text-slate-700 leading-relaxed">{selected.explanation}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function WaterFireSubSection() {
  ...
  async function handleWaterClick() {
    ...
    setSelectedId(next);
    if (next === 'thales') {
      await waterControls.start({ y: [0, -12, 0, -8, 0], transition: { duration: 1.0, ease: 'easeInOut' } });
    }
  }
  ...
  async function handleFireClick() {
    ...
    setSelectedId(next);
    if (next === 'heraclitus') {
      await fireControls.start({ scale: [1, 1.12, 0.96, 1.08, 1], opacity: [1, 0.75, 1, 0.8, 1], transition: { duration: 0.9, ease: 'easeInOut' } });
    }
  }
      ...
      <AnimatePresence mode="wait">
        {selected && (
          <motion.div key={selected.id}
            ...
            <h4 className={`text-lg font-bold mb-3 ${selected.id === 'thales' ? 'text-blue-800' : 'text-orange-800'}`}>{selected.name}</h4>
            <p className="text-slate-700 leading-relaxed text-sm">{selected.explanation}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function AtomSimulationSubSection() {
  ...
  function click(shape: string) { setActive(shape); setShow(true); }

  return (
    <div>
      <div className="flex justify-center mb-8">
        <div className="relative flex items-center justify-center w-44 h-44">
          <div className="absolute z-10 w-12 h-12 rounded-full bg-blue-900 border-3 border-blue-400 shadow-xl shadow-blue-900/30 flex items-center justify-center">
            <span className="text-white font-bold text-sm">+</span>
          </div>
          <div className="absolute w-40 h-40 rounded-full border border-blue-200" />

          <div className="absolute w-40 h-40 flex items-center justify-center orbit-slow">
            <motion.button onClick={() => click('circle')} aria-label="Hạt tròn"
              whileHover={{ scale: 1.5 }} whileTap={{ scale: 0.9 }}
              style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)' }}
              className="w-6 h-6 rounded-full bg-amber-400 border-2 border-amber-300 shadow-md cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400" />
          </div>

          <div className="absolute w-40 h-40 flex items-center justify-center orbit-medium">
            <motion.button onClick={() => click('square')} aria-label="Hạt vuông"
              whileHover={{ scale: 1.5 }} whileTap={{ scale: 0.9 }}
              style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)' }}
              className="w-5 h-5 rounded-sm bg-emerald-400 border-2 border-emerald-300 shadow-md cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400" />
          </div>

          <div className="absolute w-40 h-40 flex items-center justify-center orbit-fast">
            <motion.button onClick={() => click('triangle')} aria-label="Hạt tam giác"
              whileHover={{ scale: 1.5 }} whileTap={{ scale: 0.9 }}
              ...
              <span style={{ display: 'block', width: 0, height: 0, borderLeft: '10px solid transparent', borderRight: '10px solid transparent', borderBottom: '18px solid #f472b6' }} />
            </motion.button>
          </div>
        </div>
      </div>

      <p className="text-center text-sm text-slate-400 mb-4">Nhấn vào một hạt để xem giải thích của Démocrite</p>

      <AnimatePresence>
        {show && (
          <motion.div key="demo" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.3, ease: 'easeOut' }}
            className="relative bg-violet-50 border-2 border-violet-200 rounded-2xl p-6">
            <button onClick={() => { setShow(false); setActive(null); }} aria-label="Đóng"
              className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:bg-white/60 transition-colors">
              <X className="w-4 h-4" />
            </button>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-2xl">{active === 'circle' ? '⚪' : active === 'square' ? '🟩' : '🔺'}</span>
              <h4 className="font-bold text-violet-800">Démocrite · Thuyết Nguyên Tử</h4>
            </div>
            <p className="text-slate-700 text-sm leading-relaxed">{DEMOCRITUS}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-10">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              <span className={`text-xs mt-0.5 ${activeTab === tab.id ? 'text-blue-200' : 'text-slate-400'}`}>{tab.sub}</span>
            </button>
          ))}
        </div>
